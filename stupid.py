...
recent_ids = session.query(Metric.id).filter(Metric.metric_type == metric, Metric.company_id == self.cid, Metric.filter == self.company_subset).order_by(Metric.created_at).limit(5).all()



recent_ids = (
    session.query(Metric.id)
    .filter()
    )
...
recent_ids = ( session
    .query( Metric.id )
    .filter(Metric.metric_type == metric, Metric.company_id == self.cid, Metric.filter == self.company_subset)
...
recent_ids = session\
    .query(Metric.id)\
    .filter(Metric.metric_type == metric,
            Metric.company_id == self.cid,
            Metric.filter == self.company_subset)\
    .order_by(Metric.created_at)\
    .limit(5)\
    .all()
